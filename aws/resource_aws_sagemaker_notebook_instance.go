...
func resourceAwsSagemakerNotebookInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("name"); !ok {
		return fmt.Errorf("Error reading Sagemaker Notbook Instance name")
	} else {
		name = v.(string)
	}
...
func resourceAwsSagemakerNotebookInstanceDelete(d *schema.ResourceData, meta interface{}) error {
	...
	return resource.Retry(10*time.Minute, func() *resource.RetryError {
		...
		return resource.RetryableError(fmt.Errorf("[DEBUG] Waiting for Sagemaker Notebook Instance (%s) to stop", d.Id()))
	})
}
...
func stopSagemakerNotebookInstance(conn *sagemaker.SageMaker, d *schema.ResourceData, wait bool) error {
	...
	return resource.Retry(10*time.Minute, func() *resource.RetryError {
		...
		if err == nil {
			...
			if status == "Stopped" {
				log.Printf("[DEBUG] Sagemaker Notebook Instance (%s) is stopped", d.Id())
				return nil
			}
		}
